...
class PlotWithAssortValues {
    ...
    @Test
    fun comparemaxLossVsMargin() {
        val nruns = 100
        val name = "comparemaxLossVsMargin"
        val dirName = "$testdataDir/plots/betting/$name"
        val N = 50000

        val maxLoss = listOf(.50, .75, .90, )
        ...
        maxLoss.forEach { maxLoss ->
            margins.forEach { margin ->
                val clcaGenerator = ClcaSingleRoundAssortTaskGenerator(
                    N, margin, upper = 1.0, maxLoss = maxLoss,
                    parameters = mapOf("margin" to margin, "cat" to maxLoss)
                )
    ...
    @Test
    fun comparemaxLossVsErrorRate() {
        val nruns = 100
        val name = "comparemaxLossVsErrorRate"
        val dirName = "$testdataDir/plots/betting/$name"
        val N = 50000
        ...
        rates.forEach { rate ->
            maxLoss.forEach { maxLoss ->
                val clcaGenerator = ClcaSingleRoundAssortTaskGenerator(
                    N, margin, upper = 1.0, maxLoss = maxLoss, errorRates=rate,
                    parameters = mapOf("margin" to margin, "fuzzPct" to rate, "cat" to maxLoss)
                )
                tasks.add(RepeatedWorkflowRunner(nruns, clcaGenerator))
            }
        }
        ...
        // run tasks concurrently and average the results
        println("---comparemaxLossVsErrorRate running ${tasks.size} tasks nruns= $nruns")
        val results: List<WorkflowResult> = runRepeatedWorkflowsAndAverage(tasks)
        println(stopwatch.took())
        ...
        // fun showSampleSizesVsMargin(name: String, dirName: String, subtitle: String, yscale: ScaleType, catName: String = "auditType") {
        showSampleSizesVsFuzzPct(dirName, name, subtitle, scaleType=ScaleType.LogLog,  catName="maxLoss")
        sampleSizesVsFuzzPctStdDev(dirName, name, subtitle, catName="maxLoss", catfld= { category(it) })
    }
}