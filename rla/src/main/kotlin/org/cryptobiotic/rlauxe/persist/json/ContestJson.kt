...
@Serializable
data class ElectionStateJson(
    val name: String,
    val contests: List<ContestUnderAuditJson>,
    val done: Boolean,
)

fun ElectionState.publishJson() = ElectionStateJson(
    this.name,
    this.contests.map { it.publishJson() },
    this.done,
)

fun ElectionStateJson.import() = ElectionState(
    this.name,
...
fun writeElectionStateJsonFile(electionState: ElectionState, filename: String) {
    val jsonReader = Json { explicitNulls = false; ignoreUnknownKeys = true; prettyPrint = true }
    FileOutputStream(filename).use { out ->
        jsonReader.encodeToStream(electionState.publishJson(), out)
        out.close()
    }
}

fun readElectionStateJsonFile(filename: String): Result<ElectionState, ErrorMessages> {
    ...
    return try {
        Files.newInputStream(filepath, StandardOpenOption.READ).use { inp ->
            val json = jsonReader.decodeFromStream<ElectionStateJson>(inp)
