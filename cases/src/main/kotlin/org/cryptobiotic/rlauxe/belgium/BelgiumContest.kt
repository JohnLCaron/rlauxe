...
import org.cryptobiotic.rlauxe.dhondt.Fes
...
class BelgiumContest(val name: String) {
    val A: ExcelSpreadsheet
    val B: ExcelSpreadsheet

    init {
        A = ExcelSpreadsheet("/home/stormy/rla/cases/belgium/2024/CK_K_2024.xlsx")
        B = ExcelSpreadsheet("/home/stormy/rla/cases/belgium/2024/CK_CEListes_2024.xlsx")
    }
    ...
    fun readA(): Ainfo {
        ...
        val rows: RowIterator = A.readSheet(name)
        while (rows.hasNext()) {
    ...
    fun readB(): Binfo {
        ...
        val rows: List<Row?> = B.readRows("Seat_C$name")
        var rowno = 0
        ...
        rounds.forEachIndexed { ridx, round ->
            round.winners.forEachIndexed { widx, winn ->
                if (winn.isNotEmpty()) {
                    ...
                    val fes = Fes(partyId = winningContest.num, avg = count.toDouble(), seatno = seatno)
