...
class TestAboveThresholdOA {

    val f = 0.50
    val info = ContestInfo(
        name = "ABC",
        id = 0,
        choiceFunction = SocialChoiceFunction.THRESHOLD, // above threshold
        candidateNames = listToMap("A", "B", "C"),
        minFraction = f,
        nwinners = 1,
    )
    val contest = Contest(info, mapOf(1 to 15555, 2 to 14444), Nc = 30000, Ncast = 30000)
    val infos = mapOf(0 to info)
    // now we have 3 pools with avg >,=,< total average
    val cardPool1 = CardPool(
        "same", 1, false, infos,
        mapOf(0 to ContestTabulation(info, mapOf(1 to 5100, 2 to 4900), 10000)), 10000
    )
    val cardPool2 = CardPool(
        "low", 2, false, infos,
        mapOf(0 to ContestTabulation(info, mapOf(1 to 4890, 2 to 5300), 10000)), 10000
    )
    val cardPool3 = CardPool(
        "hi", 3, false, infos,
        // constructor(info: ContestInfo, votes: Map<Int, Int>, ncards: Int)
        mapOf(0 to ContestTabulation(info, mapOf(1 to 5565, 2 to 4244), 10000)), 10000
    )
    ...
    @Test
    fun showAboveThresholdMajorityOA() {
        println(contest)
        ...
        println("\nclcaAsertions")
        contestOA.clcaAssertions.forEach {
            val assorter = it.assorter
            println(assorter)
            assertTrue(assorter is AboveThreshold)
            assertEquals(1, assorter.winner())
            ...
            assertEquals(assorter.reportedMean(),poolAvg)
        }
        ...
        println("\nbassort")
            pools.forEach { pool ->
                ...
                println("  pool ${pool.poolId} mvr vote for cand ${terms[cand]} = ${oaAssorter.bassort(mvr, cvr)}")
            }
        }
...
fun makeMvrsFromPools(cardPools: List<CardPoolIF>) : List<AuditableCard> { // contestId -> candidateId -> nvotes
...
fun makeCvrsFromMvrs(mvrs: List<AuditableCard>): List<AuditableCard> {
    // remove votes for cards in the pools
    return mvrs.map { AuditableCard.removeVotesReplaceStyle(it, 1) }
}