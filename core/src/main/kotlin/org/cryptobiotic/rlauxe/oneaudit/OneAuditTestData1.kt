...
fun makeContestOA1(
    ...
    return makeContestOA1(winner, loser, cvrPercent, undervotePercent, phantomPercent, skewPct)
}
...
fun makeContestOA1(
    ...
    val result = OneAuditContest1.make(info, votesCvr, cvrNc, pools, Np=0)
...
fun makeTestMvrsScaled1(oaContest: OneAuditContest1, sampleLimit: Int, show: Boolean = false): List<Cvr> {
    if (sampleLimit < 0 || oaContest.Nc() <= sampleLimit) return makeTestMvrs1(oaContest)
...
fun makeTestMvrs1(oaContest: OneAuditContest1): List<Cvr> {
    val cvrs = mutableListOf<Cvr>()
...
fun makeTestNonPooledMvrs(oaContests: List<OneAuditContest1>): List<Cvr> {
...
// lets say all the pools have to be the same poolId
fun makeTestPooledMvrs(oaContests: List<OneAuditContest1>, poolId: Int): List<Cvr> {
...
fun checkAssorterAvgFromCards(oaContest: OneAuditContest1, cards: Iterable<AuditableCard>, show: Boolean = true, check: Boolean = true) {
    return checkAssorterAvg1(oaContest, cards.map{ it.cvr() }, show, check)
}

fun checkAssorterAvg1(oaContest: OneAuditContest1, mvrs: Iterable<Cvr>, show: Boolean = true, check: Boolean = true) {
    val contestUA: OAContestUnderAudit1 = oaContest.makeContestUnderAudit()
    val clcaAssertion = contestUA.minAssertion() as ClcaAssertion
    val clcaAssorter = clcaAssertion.cassorter as OneAuditClcaAssorter1
...
fun mergeCvrsWithPools1(mvrs1: List<Cvr>, mvrs2: List<Cvr>): List<Cvr> {
    var mvr2count = 0
