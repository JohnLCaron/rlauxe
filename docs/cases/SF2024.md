...
We ran the SF 2024 General Election 20 times (with different PRN seeds each time) for OneAudit, and compare it to a CLCA audit. 
...
Over 52 contests there are 131 assertions, each with a different margin. Here is the spread of the OneAudits reletive to 
...
fell below the .005 minimum recount margin, and was removed from both the CLCA and OneAudits. For OneAudit, contest 28 was also
...
By removing the close contests, we can greatly reduce
