...
import org.cryptobiotic.rlauxe.dhondt.RelaxedAssertions.ThresholdRiskFailure
...
class RelaxedAssertions(val dcontest: DHondtContest, val contestRound: ContestRound) {
    ...
    fun makeSeatRanges(): CandSeatRanges {
        ...
        val orgRanges = makeCandSeatRanges(dcontest, dhondtFailures.values.flatMap { it.arms })
        ...
        contestRound.assertionRounds.forEach { ar ->
            ...
            if (risk > alphaFudge && assorter !is DHondtAssorter) {
                thrashers.add(ThresholdRiskFailure(assorter, risk, nsamples))
            }
        }
    ...
    fun countContestedSeats(): Int {
        ...
        val altFailures = mutableListOf<DhondtLoserGroup>()
        if (thrashers.isNotEmpty()) {
            val alt = makeAltContest(thrashers)
            val wtf = makeAltRiskFailures(alt).values
            altFailures.addAll(wtf)
        }

        return dhondtFailures.values.sumOf { it.arms.size } + altFailures.sumOf { it.arms.size }
    }

    fun showContestedSeats(): String = buildString {
        ...
        altFailures.forEach { it.arms.forEach { append(it.toString()) } }
    }
