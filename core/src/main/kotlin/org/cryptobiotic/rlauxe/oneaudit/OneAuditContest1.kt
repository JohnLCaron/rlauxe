...
class OneAuditContest1 (
    val contest: ContestIF,         // contest or RaireContest
    ...
    companion object {
        fun make(info: ContestInfo,
            ...
            val contest = Contest(info, voteInput, Nc = Nc, Ncast = poolNc + cvrNcards)
            return OneAuditContest1(contest,  cvrVotes, cvrNcards, pools.associateBy { it.poolId })
        }
...
open class OAContestUnderAudit1(
    ...
    override fun makeClcaAssorter(assertion: Assertion, assortValueFromCvrs: Double?): ClcaAssorter {
        ...
        return OneAuditClcaAssorter1(this.contestOA, assertion.assorter)
    }
    ...
    override fun recountMargin(): Double {
        ...
        if (contestOA.contest.votes() != null) {
            val votes = contestOA.contest.votes()!!
...
class OneAuditClcaAssorter1(
    ...
    assortValueFromCvrs: Double? = null,    // Ā(c) = average CVR assorter value. TODO wrong ??
    hasStyle: Boolean = true,
    ...
    override fun bassort(mvr: Cvr, cvr: Cvr, hasStyle: Boolean): Double {
        ...
        ///// new way
        // val poolAverageNew = poolAverages.assortAverage[cvr.poolId] ?: throw IllegalStateException("Dont have pool ${cvr.poolId} in contest ${contestOA.id} assorter")
