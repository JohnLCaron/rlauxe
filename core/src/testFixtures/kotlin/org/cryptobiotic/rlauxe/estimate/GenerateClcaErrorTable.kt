...
class GenerateClcaErrorTable {
    ...
    //   using ContestTestDataBuilder from MultiContestTestData, ContestSimulation to make the cvrs
    //   using deprecated ClcaFuzzSampler (only place that uses it)
    @Test
    ...
    // this is from ClcaFuzzSamplerTracker (production)
    ...
    fun testFuzzedCardsErrors2() {
        ...
        fuzzPcts.forEach { fuzzPct ->
            ...
            val avgErrorRates = ClcaErrorRatesCumul() // avg across contests and assertions ?? or use min assertion ??
            contestsUA.forEach { contestUA ->
                contestUA.clcaAssertions.forEach { cassertion ->
                    val cassorter = cassertion.cassorter
                    ...
                    // ?? avgErrorRates.add(sampler.pluralityErrorRates())
                }
            }
            println("fuzzPct ${dfn(fuzzPct,3)}: ${avgErrorRates}")
        }
