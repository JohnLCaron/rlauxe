...
The weight of stratum k is w_k := N_k /N.

For each assorter A there is a set of assorter values {x_i, i=1.. N}.
Each assorter may have its own upper bound u_k in stratum k. why??
The true mean of the assorter values in stratum k is µ_k ; µ := [µ1 , ..., µK ] whats T transpose?
The overall assorter mean µ = wT dot µ.

Let θ = [θ_1 , ..., θ_K ] with 0 ≤ θ_k ≤ u_k
A single intersection null is of the form µ ≤ θ, i.e., ∩ k=1..K { µ_k ≤ θ_k }.
The union-intersection form of the complementary null that the outcome is incorrect is:

        H0:   U  wT dot µ < 1/2   {    ∩ k=1..K { µ_k ≤ θ_k }    }          eq (1)
...
Let uk_A be the upper bound on the original(primitive) assorter for stratum k and
...
Letting u_k := 2uA_k, take B_ik := uA_k − ω_ik ∈ [0, uk ] and B̄k := Sum { B_ik } / N_k
Then { B_ik } is a bounded list of nonnegative numbers, and the assertion in stratum
k is true if 
...
can reject all intersection nulls θ for which 0 ≤ θ ≤ u and w dot θ ≤ 1/2.
...
whether the P -value of any intersection null in the union is greater than the risk
limit.
...
Equivalently, P(θ) maximized over feasible θ is a P-value for eq (1):
    ...
    P∗ := max {P(θ): 0 ≤ θ ≤ u and w dot θ ≤ 1/2}.
...
3.4 Fisher's Combining Functions
...
Possibilities include SUITE, ALPHa, Empirical Berstein, and Betting (see below).
...
previous implementations of SUITE only work for K = 2. However, the combined
log-P -value for EB is linear in θ for PM*
and piecewise linear for PF∗ . Maximizing
the combined log-P -value over the union of intersections is then a linear program
...
Sequentially valid inference, or "anytime-valid" inference.
Traditional statistics require fixing the sample size (N) before collecting data. 
If one "peaks" at the data and stops early, they are more likely to find a false positive (type-I error). 
...
The population mean is µ(X), and we want to test the global null hypothesis:
      ...
      H0 : µ(X ) ≤ η0  for global null mean η0.    ( eq 1)

A lower 1 − α confidence bound is the largest η0 for which H0 is not rejected at level α. 
...
Let ℵ0_N := {X_N ∈ ℵ_N : µ(X_N ) ≤ η0 } denote the set of null populations; 
the global null hypothesis can be written H0 : X*_N  ∈ ℵ0.
Let ℵ1_N := {X_N ∈ ℵ_N : µ(X_N ) >= η0 } denote the alternative populations;
together ℵ0_N and ℵ1_N partition ℵ_N.

Let µ* := µ(X*_N) be the true global mean, and µ** := [µ*_1, ... , µ*_k ] be the true stratumwise means.
An intersection null hypothesis is the assertion µ** ≤ η for the intersection null mean η ∈ [0, 1]^K.
The global null hypothesis can be written as a union of intersection null hypotheses:

      H0 : U {η ∈ E0 µ** ≤ η}                      (eq 2)

where E0 := {ζ : w · ζ ≤ η0 , 0 ≤ ζ ≤ 1} is the set of all intersection nulls for which the global null is true.

3.2 Sampling design (v.1 paper)
...
and (X_ki), i=1..N is a random permutation of the stratum values {x_ki k=1..N_n}

An interleaving of samples across strata is a stochastic process (Y_t) indexed by discrete
time t; Y^t := (Y_i )i=1..t is the t-prefix of (Y_i)i∈N.
An interleaving is characterized by within-stratum data and a stratum selection S_t : the item in the tth position in the interleaving comes
...
As of time t, the sampling depth in stratum k is the number of items in Y^t that came from stratum k, denoted T_k(t).
...
For instance, S^t(η) is a sequence of selections for η and Y^t(η) is the corresponding interleaving.
The filtration F_t(η) := σ(Y^t(η), S^(t+1)(η)) is the sigma-field representing everything known
up to time t at null η.

2.3.1 Constructing TSMs from random samples.

Consider a single stratum with true
mean µ_k and null mean η_k . We construct a process (Mkt )t∈N that is a TSM with respect to the
samples (Xkt )t∈N under the stratum null µ⋆k ≤ ηk . The conditional stratumwise null mean η_kt
is the mean of the values remaining in X_k at time t if the null is true. (n_kt is "populationMeanIfH0")

In particular, we use the betting TSM

      M_kt(η_k) := Prod { (1 + λ_ki (X_ki − η_ki )), i=1..T_k(t)) }  (this is rlauxe betting risk-function)
      M_kt(η_k) := Prod { (1 + λ_ki (X_ki − eta_ki )), i=1..T_k(t)) }  (eq 2.3.1) (switch to eta with replacement notation)

Within-stratum TSMs can be combined across strata to form a P -value for an intersection null.

2.3.2 Intersection test supermartingales (I-TSMs)

      M_t(η) := Prod { M_kt(η_k), k=1..K } = Prod { Prod { Z_ki(η_k), i=1..T_k(t))}, k=1..K  } = Prod { Z̃i }

where

      Z̃i = Z_Si,T_Si(i) (η_S_i ) is the TSi(i)th term of the within-stratum TSM for stratum S_i
...
3.1 Simple stratified inference: combining confidence bounds (LSB)
...
of E0 above. As shown in Section 2.3.2, we can test a particular η ∈ E0 using an I-TSM.
We can reject H0 if the P -value for every η ∈ E0 is less than α, i.e., if the smallest I-TSM
evaluated over E0 is at least 1/α.

      Mt := min max Mj(η) = [max Pt(η)]^-1 = max [max [Mj(η)]^-1 ]^-1

is a UI-TS for H0 . When every Mt(η) is a betting I-TSM, Mt is a betting UI-TS.

3.2.1. The boundary of E0. 

Minimizing over E0 is non-trivial. We restrict attention to within-stratum TSMs that are always monotone decreasing in ηk .
Consequently, the minimum of Mt (η) occurs on the boundary of E0 , i.e., the set of points in the null that are closest to the alternative.

3.2.2. Global stopping time and global sample size. 

We distinguish between the stopping time of a UI-TS and its workload, measured by the number of samples it requires to stop.
The stopping time of the level α test
induced by UI-TS Mt at η is τ (Mt ; η) := min{t ∈ N : Mt (η) ≥ 1/α}. This is the number
of overall samples needed for Mt (η) to reject η ; and τk (Mt ; η) := Tk (τ (Mt ; η), η) is the
number of samples needed from stratum k . The global stopping time is the sample size needed
for the “last” I-TSM, considered on its own, to hit or cross 1/α:

      τ (Mt ) := inf{t ∈ N : Mt ≥ 1/α} = sup η∈C Sum { τk (Mt;η), k=1..K }

On the other hand, the global sample size nτ is the total number of samples drawn across all
strata when H0 is rejected:

      n_τ (Mt) := Sum { sup η∈C τk (Mt;η), k=1..K }
...
We will call a UI-TS consistent if it produces a globally consistent test. A UI-TS must grow
over the entire set of selections {St(η)}η∈C to be consistent, and this set is included in the
aforementioned stochastic aspects of the test. In theory, we can use any set of consistent
I-TSMs {Mt(η)}η∈C to construct a consistent UI-TS. In practice, C may be uncountably
infinite, so computing the UI-TS P -value Pt may be intractable unless the set {Mt (η)}η∈C
has some structure. Perhaps surprisingly, the simplest ways to structure {Mt (η)}η∈C —fix
...
4.2. Efficiency. Practically useful tests should keep nτ relatively small when the null
...
(median, expected value, etc) should be minimized; (c) for a UI-TS, nτ can have a complicated
structure depending on the underlying I-TSMs.

Recall that Lemma 1 relates the sample size and stopping time as τ ≤ nτ ≤ Kτ for general
...
These tests are called stopping-time optimal (STO).

STO UI-TSs can be constructed as the minimum of a collection of Kelly-optimal I-TSMs. See Definition 7

We also consider constraining the UI-TS to use the same stratum selections (St ) for all η ∈ E0 , in which case nτ = τ by Lemma 1. 
We call such UI-TSs STO for (St). See Definition 8
...
The predictable plug-in strategy λP_kt := 1 ∧ sqrt(2log(2/α)/(σ̂_k(t−1)^2 * t * logt),
recommended by Waudby-Smith and Ramdas[2024], is an instance of a fixed bet when truncated to [0, 1] rather than [0, 1/ηk].
...
λInverse_kt(η) produces a set of I-TSMs that are convex over η ∈ C, making the UI-TS more computationally tractable

5. Computation

In principle, the results of Section 4.2.1 allow one to construct an
approximately optimal I-TSM for every η ∈ C ; Lemma 3 then constructs a STO UI-TS under
the alternative µ⋆ > η0 . In practice, this can be infeasible to implement. The tractability of the
minimization depends on the constituent I-TSMs and how they vary over η ∈ C . We classify
I-TSMs accordingly. The term η -aware refers to betting and selection strategies that depend on
η ; η -oblivious refers to strategies that do not. An I-TSM or UI-TS is called η -oblivious only
if all its selectors and bets are η -oblivious. We now describe a few strategies for computation
...
Using the stratified CCA parametrization in Spertus and Stark [2022, §3.2], we constructed populations with stratum
...
as η_k := (1 − θ_k − Āc_k)/2 where Ā_ck was the stratumwise reported assorter mean—the share
of votes for the winner—and θ ∈ {θ : w · θ = 1/2, 0 ≤ θ ≤ 1}. Letting Āc := [Āc_1 , Āc_2], the
global reported assorter mean Ā_c := w · Ā ranged between 0.51 and 0.75, corresponding to
...
η-oblivious, nonadaptive round-robin, an η-aware and adaptive “predictable Kelly” strategy
using a UCB-style algorithm on every η ∈ C, or an η-oblivious “greedy” strategy using the
UCB-style algorithm iteratively on the minimizing η at time (t − 1) to produce a single
interleaving for all I-TSMs.

Figure 3 gives sample sizes for G = 100. 

UI-TSs are generally more efficient than LCBs. The difference is substantial (the y-axis is on the log scale)
except when neither the bets nor selections are η-aware (e.g., predictable plug-in bets with
...
Greedy selection and round robin performed similarly, except when the stratum gap was large and the bets were fixed

UI-TS with either Agrapa or inverse; Greedy selection or round robin.

Start with inverse bet and 
