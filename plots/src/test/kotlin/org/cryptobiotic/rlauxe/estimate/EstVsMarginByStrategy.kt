...
class EstVsMarginByStrategy {
    ...
    fun estSamplesVsMarginByStrategy() {
        ...
        val auditConfig = AuditConfig(AuditType.CARD_COMPARISON, true, seed = Random.nextLong(), quantile=.80, ntrials = 100, clcaConfig = clcaConfig)

        val tasks = mutableListOf<ConcurrentTaskG<List<WorkflowResult>>>()

        margins.forEach { margin ->
            /* val clcaGenerator1 = ClcaWorkflowTaskGenerator(N, margin, 0.0, 0.0, fuzzPct,
                parameters=mapOf("nruns" to nruns.toDouble(), "strat" to 1.0, "fuzzPct" to fuzzPct),
                auditConfigIn=auditConfig.copy(clcaConfig=ClcaConfig(ClcaStrategyType.oracle, fuzzPct))
            )
