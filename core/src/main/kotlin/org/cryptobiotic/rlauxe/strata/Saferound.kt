...
// Standard rounding might result in a sum of 10 or 11
//  saferound ensures the sum is exactly 10
// data = [3.34, 3.33, 3.33]
// rounded = saferound(data, places=0)
// # Output: [4, 3, 3] (Sum = 10)
// You need to use saferound (from the iteround library) when rounding a list or array of numbers where the sum of the
// rounded values must precisely equal the rounded total of the original sum.
// from our SpecialFriend

fun saferound(values: List<Double>, places: Int = 0): List<Int> {
    val multiplier = Math.pow(10.0, places.toDouble()).toInt()

    // 1. Initial rounded values (floor)
    var rounded = values.map { floor(it * multiplier).toInt() }

    // 2. Calculate remaining sum needed
    val targetSum = (values.sum() * multiplier).roundToInt()
    val currentSum = rounded.sum()
    val difference = targetSum - currentSum

    if (difference == 0) return rounded

    // 3. Find indices to add remainder to (highest fractional parts)
    val sortedIndices = values.indices.sortedByDescending { i ->
        (values[i] * multiplier) - floor(values[i] * multiplier)
    }

    // 4. Distribute difference
    val result = rounded.toMutableList()
    for (i in 0 until difference) {
        val indexToIncrement = sortedIndices[i]
