...
fun simulateRaireTestData(contest: RaireContest, quiet: Boolean = true): List<Cvr> {
    val minMargin = 0.0
    repeat(11) {
...
fun simulateRaireTestData(N: Int, contestId: Int, ncands:Int, minMargin: Double, undervotePct: Double = .05, phantomPct: Double = .005, quiet: Boolean = true): Pair<RaireContestUnderAudit, List<Cvr>> {
    repeat(11) {
...
data class RaireContestTestData(
    ...
    override fun toString() = buildString {
        append(
            "ContestTestData($contestId, ncands=$ncands, ncards=$ncards, undervotePct=${df(undervotePct)} phantomPct=${
                df(
                    phantomPct
                )
            } Nc=$Nc"
