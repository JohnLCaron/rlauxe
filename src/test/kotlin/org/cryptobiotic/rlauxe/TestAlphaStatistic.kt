...
class TestAlphaStatistic {
    ...
    fun testWithSampleMean() { // Seems stupid not to use the sample mean for eta0:
        ...
        randomMeans.forEach { ratio ->
            ...
            val result = runAlphaGen(sampler, m, genRatio=ratio, nrepeat = nrepeat)
...
fun runAlphaGen(
    ...
    val estimFn =
        TruncShrinkage(N, upperBound, t, minsd = minsd, d = d, eta0 = eta0, f = f, c = c, eps = eps)
    ...
    repeat(nrepeat) {
        drawSample.reset()
        val testH0Result = alpha.testH0(maxSamples) { drawSample.sample() }
...
class TruncShrinkage(
    val N: Int, val u: Double, val t: Double, val minsd: Double, val d: Int, val eta0: Double,
    val f: Double, val c: Double, val eps: Double
) : EstimFn {
    val welford = Welford()
