...
fun merge(cards: List<CardWithStyleName>, styles: List<StyleIF>?) : List<AuditableCard> {
    val iter = MergeStylesIntoCards(Closer(cards.iterator()), styles ?: emptyList())
...
class MergeBatchesIntoCardManifestIterable(
    ...
    override fun iterator(): CloseableIterator<AuditableCard> = MergeBatchesIterator(cards.iterator(), styles)
    ...
    private class MergeBatchesIterator(
        ...
        // styleName must be in styleMap
        override fun next(): AuditableCard {
            val org: CardWithStyleName = cardsIter.next()
            val style = styleMap[org.styleName]
