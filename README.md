...
[published papers](#primary-reference-papers) of Stark et al.
...
3. CVRS are not available, but batch subtotals are available from which OneAudit pools can be created.
...
These results are generated by our single round (no estimation phase) CLCA algorithm with maxRisk = 90%, and would be different with other choices of algorithm parameters. (A least squares plot would be more accurate but we don't actually use this approximate fit in our algorithms.)
...
because the card knows if it's a phantom, or if it belongs to a OneAudit pool. Then:
...
Each round does its own consistent sampling without regard to the previous round's results. Previously audited MVRS are always used again in subsequent rounds, for contests that continue to the next round. The auditors only have to find and hand audit the _new mvrs_ that haven't been audited before.
...
MaxLoss is a user settable parameter which limits the maximum bet that can be placed on any one sample.
How to set MaxLoss in an optimal way?

**Cost model**

A cost model is needed to optimize tradeoffs like samples size vs number of rounds. Or batch audit vs a OneAudit.
Allow a user to set different cost estimates and run "what if" scenarios.

Possible cost estimates:
* cost to retrieve K cards from a batch of N cards ?
* cost for a team to examine one ballot and enter their finding into a spreadsheet or data input form ?
* cost to hand count a batch of size N, and enter the subtotal into a spreadsheet or data input form ?
* fixed cost to run the first round of an audit ?
* fixed cost to run subsequent rounds of an audit ?

Specify amounts in dollars, but the main result would be relative costs, eg the ratio of the cost of a batch audit vs a OneAudit.
...
* [Compare with Shangrla library (generated by Copilot agent)](docs/ShangrlaComparison.md)
