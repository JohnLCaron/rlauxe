...
* 4224 pools with 216286 cards (13.5%), using SHANGRLA grouping (session.TabulatorId-session.BatchId) aka "Session-Scan".
* 2565 pools with 216286 cards (13.5%), grouped by (precinct-cardStyle) aka "Precinct-Style".
...
Since we have the CVRs, we can create subtotals for whatever subset we want. The difficult part is to create physical batches
...
the random seed is chosen. Then the ordering must match the CardManifest and must be preserved.
...
We use the SF2024 case study to characterize the increased nmvrs needed for both kinds of OneAudits in a real-world example.
We can easily simulate a CLCA for SF2024 by simply pretending that the precincts record the CVRS on the physical ballots,
and so all CVRS can be matched to MVRs.

## Comparison of CLCA and OneAudits

We ran the SF 2024 General Election 20 times (with different PRN seeds each time) for Session-Scan OneAudit, Precinct-Style OneAudit, and a single CLCA audit. 
...
might go to a full hand count immediately. For OneAudit, close contests may fail to complete when they exceed sample control constraints.
...
We use 20 runs to generate the distributions, and then show the average and stddev of the distributions:
...
The corresponding average amd stddev of the number of rounds needed for the two OneAudits. CLCA without errors will always just use one round.
...
* Precinct Style generally does better than Session Scan, possibly because of the reduced variance.
...
## Previous results with Session Scan OneAudit (somewhat outdated)
...
as its _possibleContests_, and _hasExactContests_ = false. This greatly increases the population size for each contest, which decreases
...
* run makeSFElectionClca() to create a CLCA election in  _$testdataDir/cases/sf2024/clca/audit_
* run makeSFElectionOA() to create a Session-Scan OneAudit election in  _$testdataDir/cases/sf2024/oa/audit_
* run makePrecinctAndStyleOA() to create a Precinct-Style OneAudit election in  _$testdataDir/cases/sf2024/oasp/audit_
