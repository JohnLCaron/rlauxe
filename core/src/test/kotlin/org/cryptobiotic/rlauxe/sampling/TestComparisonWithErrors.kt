...
class TestComparisonWithErrors {
    ...
    fun testFuzzTwoPersonContest() {
        ...
        var sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrsFuzzed.zip(cvrsUI), assort, allowReset = true)
        ...
        sampler = ComparisonWithoutReplacement(contestUA.contest, mvrsOmit.zip(cvrsUI), assort, allowReset = true)
    ...
    fun testFuzzThreePersonContest() {
        ...
        var sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrsFlip.zip(cvrsUI), assort, allowReset = true)
        var samples = PrevSamplesWithRates( assort.noerror)
        repeat(ncvrs) { samples.addSample( sampler.sample())}
        ...
        // omit
        val mvrsOmit = cvrsUI.map { it.omit() }
        sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrsOmit.zip(cvrsUI), assort, allowReset = true)
        samples = PrevSamplesWithRates( assort.noerror)
        repeat(ncvrs) { samples.addSample( sampler.sample())}
        ...
        sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrsOther.zip(cvrsUI), assort, allowReset = true)
        samples = PrevSamplesWithRates( assort.noerror)
        repeat(ncvrs) { samples.addSample( sampler.sample())}
        ...
        sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrs1.zip(cvrsUI), assort, allowReset = true)
        samples = PrevSamplesWithRates( assort.noerror)
        repeat(ncvrs) { samples.addSample( sampler.sample())}
        ...
        sampler = ComparisonWithoutReplacement(contestUA.contest as Contest, mvrs0.zip(cvrsUI), assort, allowReset = true)
        samples = PrevSamplesWithRates( assort.noerror)
        repeat(ncvrs) { samples.addSample( sampler.sample())}
